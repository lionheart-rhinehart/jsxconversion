...
// Per-asset data: window.__CONFIG__ (set by an injected <script> before this
// runs) is passed as the `data` prop, so data-driven templates like
// review-roundup.jsx render campaign copy instead of their defaults. Templates
// that ignore `data` are unaffected (React drops unused props).
...
(function () {
  function start() {
    ...
    var data = window.__CONFIG__ || {};
    window.__rerender = function () {
      window.__rerenderCount++;
      root.render(React.createElement(Comp, { data: data }));
    };
    window.__rerender();
    window.__ready = true;
  }
...
export async function renderShippedClaudeDesign({
  ...
  // Optional per-asset data: a sibling "<name>.data.json" next to the input
  // JSX. The campaign runner writes one per asset so the same bank template
  // renders different campaign copy. Absent → {} → templates use their own
  // defaults (identical to pre-injection behavior).
  const dataPath = inputPath.replace(/\.(jsx|tsx)$/, ".data.json");
  let configData = {};
  if (existsSync(dataPath)) {
    try {
      configData = JSON.parse(readFileSync(dataPath, "utf8"));
    } catch (e) {
      console.error(`[render] warning: could not parse ${dataPath}: ${e.message}`);
    }
  }

  // Shared dependency files a bank template may rely on. Claude Design bank
  // templates (brand/video-templates/templates/*.jsx) reference shared
  // elements (window.StarRating, etc.) defined in sibling files. animations.jsx
  // is loaded explicitly below; here we also pull in editing.jsx and every
  // elements/*.jsx if present, so element-dependent templates don't render
  // with missing sub-components. Single-file projects (no elements/ dir) add
  // nothing and behave exactly as before.
  const depScripts = [];
  const editingPath = join(projectDir, "editing.jsx");
  if (existsSync(editingPath)) {
    depScripts.push(`/${encodeURIComponent(relativeToProject(editingPath, projectDir))}`);
  }
  const elementsDir = join(projectDir, "elements");
  if (existsSync(elementsDir)) {
    for (const f of readdirSync(elementsDir).filter((n) => /\.(jsx|tsx)$/.test(n)).sort()) {
      depScripts.push(`/elements/${encodeURIComponent(f)}`);
    }
  }
  ...
  // driver shim, shared deps (editing.jsx + elements/*), the per-asset
  // __CONFIG__, the variation, and the bootstrap script in order.
  ...
  const depScriptTags = depScripts
    .map((s) => `<script type="text/babel" data-presets="env,react" src="${s}"></script>`)
    .join("\n");
  const configTag = `<script>window.__CONFIG__ = ${JSON.stringify(configData)};</script>`;
...
function relativeToProject(absPath, projectDir) {
  // Use path.relative so Windows backslash separators are handled, then
  // normalize to forward slashes for the URL the static server expects.
