...
1. **Locate the input file.** If the user pasted JSX inline, write it to
   `examples/<short-name>.jsx` first.
2. **Ensure deps are installed.** Run `npm install` if `node_modules/` is
   ...
   cloud session the SessionStart hook handles ffmpeg; locally, ask the user
   to install if missing.
3. **Run the renderer:**
   ```
   node .claude/skills/jsx-to-mp4/scripts/render.mjs <path-to-jsx>
   ```
   The script handles classification (Remotion / animated / static),
   parameter resolution, bundling, capture, and ffmpeg encoding.
4. **Verify the output.** Check `out/<name>.mp4` exists and is non-empty.
   Report duration, resolution, and fps from the script's stdout to the user.
5. **Hand off.** Use `SendUserFile` to deliver the MP4 with a one-line
...
- **Fonts:** custom fonts must be `@import`ed via Google Fonts URL or
  loaded with `<link>` inside the component — local font files won't be
  found by the bundler unless added explicitly.
