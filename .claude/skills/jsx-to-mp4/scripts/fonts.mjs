...
// Parse a shipped Google Fonts <link> URL into the families it covers,
// keeping the per-family axis spec verbatim. The axis spec is critical:
// it tells Google Fonts whether to serve a variable font (axis ranges
// with ".." syntax, e.g. "opsz,wght@0,9..144,400..800") or static
// instances at named weight/style tuples. Stripping it forces a
// default static request, which silently downgrades variable fonts like
// Fraunces — at large display sizes the rendered glyph metrics no
// longer match the design preview.
//
// Returns: [{ family: string, axisSpec: string|null }]
//   axisSpec includes the leading ":" if present, or is null for bare
//   family references like "Anton" or "Archivo Black".
export function familiesInGoogleFontsUrl(url) {
  ...
  while ((m = re.exec(url)) !== null) {
    ...
    if (colon === -1) {
      out.push({ family: raw.replace(/\+/g, " "), axisSpec: null });
    } else {
      out.push({
        family: raw.slice(0, colon).replace(/\+/g, " "),
        axisSpec: raw.slice(colon), // includes the leading ":"
      });
    }
  }
  return out;
...
async function fetchGoogleFontsCss(family, axisSpec) {
  const param = family.replace(/\s+/g, "+");
  // When the shipped design's HTML <link> already carries an axis spec,
  // pass it through verbatim. This is the only way to ask Google Fonts
  // for the variable font (e.g. Fraunces opsz axis) instead of static
  // instances pinned to default opsz, and also the only way to get
  // exact weights for families with non-default available weights
  // (e.g. Caveat only ships 400/500/700).
  const spec = axisSpec ||
    ":ital,wght@0,400;0,500;0,600;0,700;0,900;1,400;1,700";
  const url =
    `https://fonts.googleapis.com/css2?family=${param}${spec}&display=swap`;
...
async function downloadFromGoogleFonts(family, axisSpec) {
  const r = await fetchGoogleFontsCss(family, axisSpec);
...
export async function preflightFonts({ projectDir, jsxFiles, shippedHtmlPath }) {
  ...
  // family-lowercase → axisSpec (or null for bare-family entries).
  // Membership doubles as the "is this covered by shipped link?" check.
  const coveredAxes = new Map();
  if (shippedHtmlPath && existsSync(shippedHtmlPath)) {
    ...
    if (shippedFontsUrl) {
      for (const { family, axisSpec } of familiesInGoogleFontsUrl(shippedFontsUrl)) {
        coveredAxes.set(family.toLowerCase(), axisSpec);
      }
    }
  }
  ...
  for (const family of referenced) {
    ...
    if (coveredAxes.has(family.toLowerCase())) {
      const axisSpec = coveredAxes.get(family.toLowerCase());
      ...
      let downloaded;
      try {
        downloaded = await downloadFromGoogleFonts(family, axisSpec);
      } catch (e) {
