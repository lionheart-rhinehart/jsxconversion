...
- `<name>.config.json` — the data-driven layer model: `{ width, height, media?,
  fixedDesign[], elements[], foregroundMedia? }`. Each editable layer carries a
  semantic `tag`. Validate JSON before writing.
...
fallbacks, plus `window.XReel = XReel` and an optional `X_SPEC = { fields: [...] }`
