...
export function renderDesignItem(it, key) {
  if (it.type === "rect") {
    ...
    // Motion (Part 3): keyframe animation on the shape (gauges/scanlines). Its
    // transform-origin + keyframe custom-props (--low…) ride along. Inert when absent.
    if (it.animation) {
      rectStyle.animation = it.animation;
      if (it.animTransformOrigin) rectStyle.transformOrigin = it.animTransformOrigin;
      if (it.animVars) Object.assign(rectStyle, it.animVars);
    }
  ...
  // ── svg: raw inline-SVG device passthrough (Westfield) ─────────────────────
  // Captured verbatim from the design (EKG line draw, guarantee ring…) at its
  // frozen rest state. Positioned absolutely at its design box. Inert for every
  // other template (none set type:"svg").
  if (it.type === "svg") {
    return (
      <div
        key={key}
        style={{ position: "absolute", left: it.x, top: it.y, width: it.width, height: it.height, opacity: it.opacity ?? 1 }}
...
export function renderTextLayer(el, key) {
  ...
  // Motion (Part 3): the keyframe animation rides an INNER span (so an outer
  // position/rotate stays clean — mirrors the editor). transform-origin + keyframe
  // custom-props (--low…) ride the span. Count-up spans carry data-* the render's
  // count-up driver reads. All inert when absent → static text byte-identical.
  const animStyle = el.animation
    ? { animation: el.animation, display: "inline-block", ...(el.animTransformOrigin ? { transformOrigin: el.animTransformOrigin } : {}), ...(el.animVars || {}) }
    : null;
  const cuAttrs = el.countup
    ? { "data-countup": el.countup.target, "data-decimals": el.countup.decimals || 0, "data-prefix": el.countup.prefix || "", "data-suffix": el.countup.suffix || "" }
    : {};
  if (el.chipBg) {
    ...
    const innerStyle = {
      ...
      ...(el.animation ? { animation: el.animation, ...(el.animTransformOrigin ? { transformOrigin: el.animTransformOrigin } : {}), ...(el.animVars || {}) } : {}),
    };
    return (
      <div key={key} style={outerStyle}>
        <span style={innerStyle} {...cuAttrs}>{content}</span>
      </div>
    );
  }
  if (el.animation || el.countup) {
    // Apply the animation DIRECTLY on the positioned div (layout-neutral — no inner
    // box that would shift a stacked headline; the render has no moveable to fight
    // the transform). data-countup rides the same div for the count-up driver.
    const s2 = { ...style };
    if (el.animation) {
      s2.animation = el.animation;
      if (el.animTransformOrigin) s2.transformOrigin = el.animTransformOrigin;
      if (el.animVars) Object.assign(s2, el.animVars);
    }
    return (
      <div key={key} style={s2} {...cuAttrs}>{content}</div>
    );
  }
  return (
...
export function LayerStack({ config, background }) {
  ...
  return (
    <Frame width={W} height={H} background={background || INK_950}>
      {/* Motion (Part 3): inject this design's @keyframes library so the animated
          layers actually move (editor preview + render share this one source). */}
      {config.keyframes ? <style dangerouslySetInnerHTML={{ __html: config.keyframes }} /> : null}
      {layers.map((layer, idx) => {
        ...
        if (layer.kind === "media") {
          ...
          // Bounded media (Route B partial photo): render inside its frame box,
          // clipped — instead of full-bleed. Inert when `frame` is absent.
          if (f) {
            return (
              <div key={key} style={{ position: "absolute", left: f.x, top: f.y, width: f.width, height: f.height, overflow: "hidden" }}>
                {slot}
              </div>
            );
          }
