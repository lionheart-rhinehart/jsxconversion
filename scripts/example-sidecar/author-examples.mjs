...
function main() {
  ...
  const manifest = { note: "Track-B authoring manifest — generated by author-examples.mjs. 15 archetypes × 3 sub-looks (45 examples). Edit the generator's ARCHETYPES, not this file.", examples: [] };
  let seq = 0;
  ...
  for (const a of ARCHETYPES) {
    for (const v of a.variants) {
      ...
      manifest.examples.push({ id, archetype: a.archetype, format: "static", mediaStyleAccepts: v.accepts || [], slotShape: v.slotShape || a.slotShape });
    }
  }

  writeFileSync(MANIFEST, JSON.stringify(manifest, null, 2) + "\n");
  process.stderr.write(`[author] wrote ${manifest.examples.length} examples (${ARCHETYPES.length} archetypes × 3 sub-looks)\n`);
}
