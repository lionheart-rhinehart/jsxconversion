// One-off: clone the confidence (proof-confidence) set into 3 AA-location
// campaigns, swapping ONLY the location token (INDIANAPOLIS → city) wherever it
// appears: static eyebrows + E4 byline (edit configs) and E2/E4 templateData
// (plan). Nothing else changes.
import { readFileSync, writeFileSync, mkdirSync, existsSync, readdirSync, copyFileSync } from "node:fs";
import { join } from "node:path";

const ROOT = process.cwd();
const SRC = join(ROOT, "campaigns", "multisport-foundations-confidence");
const LOCS = [
...
for (const loc of LOCS) {
  const name = `confidence-${loc.slug}`;
  const dest = join(ROOT, "campaigns", name);
  mkdirSync(join(dest, "edits"), { recursive: true });

  // plan: rename, set location, reset statuses, swap templateData location token
  ...
  writeFileSync(join(dest, "kraken.json"), JSON.stringify({
    workspace: loc.slug, workspaceId: loc.wsId, destFolderId: null,
  }, null, 2));

  console.log(`${name}: ${copied} edit configs (${cfgSwaps} text swaps), ${tdSwaps} templateData swaps, kraken ws=${loc.slug}`);
}
console.log("done");
