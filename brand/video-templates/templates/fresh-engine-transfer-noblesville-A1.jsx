// FRESH-ENGINE-TRANSFER-NOBLESVILLE-A1 — AA "action-hero" cold hook (beat A).
// Built from scratch on the Athletes Acceleration design system, guided by example
// ex-056-action-hero: full-bleed athlete sprint clip with a slow Ken-Burns push-in, a
// left-edge red accent column that wipes down, a mono eyebrow chip top-left, and a HUGE
// Anton headline owning the lower third (the dominant element). Heavy bottom scrim for
// legibility, AA wordmark bottom-left. Vertical-native 1080x1920. All copy via data.*;
// bg clip via data.bgClip (run-campaign maps asset.clip -> bgClip and pre-extracts frames).

function FreshEngineTransferNoblesvilleA1Reel({ data = {} }) {
  const eyebrow = data.eyebrow ?? 'NOBLESVILLE SPORT PARENTS';
  const hook = data.hook ?? 'HOW TRAINING TRANSFERS TO EVERY SPORT';
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: INK, overflow: 'hidden' }}>
      {bgClip ? (
        <SyncedVideo src={bgClip}
          style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover',
            transform: `scale(${push})`, transformOrigin: '55% 40%' }} />
      ) : null}
      ...
      {/* dominant headline, lower third */}
      <TplText field="hook" data={data}
        base={{ position: 'absolute', left: 96, right: 84, bottom: 286 }}
        style={{ fontFamily: '"Anton", "Oswald", sans-serif', fontWeight: 400, fontSize: 132, color: '#ffffff',
...
window.FreshEngineTransferNoblesvilleA1Reel = FreshEngineTransferNoblesvilleA1Reel;

const FRESH_ENGINE_TRANSFER_NOBLESVILLE_A1_SPEC = {
  id: 'fresh-engine-transfer-noblesville-A1',
  name: 'ENGINE-TRANSFER A1 — ACTION HERO',
  fields: [
    ...
    { "key": "hook", "role": "hook", "label": "Hook", "type": "text", "default": "HOW TRAINING TRANSFERS TO EVERY SPORT" },
    { "key": "brand", "role": "brand", "label": "Brand", "type": "text", "default": "ATHLETES ACCELERATION" }
  ],
};
window.FRESH_ENGINE_TRANSFER_NOBLESVILLE_A1_SPEC = FRESH_ENGINE_TRANSFER_NOBLESVILLE_A1_SPEC;
